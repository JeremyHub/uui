    ...
    (function backfill(parent) {
        [...parent.children].forEach((child, i) => {
            ...
            const guess = { HEADER: "header", NAV: "nav", FOOTER: "footer", MAIN: "main",
                ASIDE: "sidebar", FORM: "form" }[child.tagName] ||
                `block-${i + 1}`;
            let name = guess, n = 2;
            while (used.has(name))
                name = `${guess}-${n++}`;
...
export function splitOversizedRegions(doc) {
    ...
    const used = new Set([...doc.querySelectorAll("[data-region]")].map((el) => el.getAttribute("data-region")));
    for (const region of [...doc.querySelectorAll("[data-region]")]) {
        if (doc.querySelectorAll("[data-region]").length >= MAX_REGIONS)
            break;
        if (region.innerHTML.length / total < OVERSIZED_SHARE)
            continue;
        ...
        if (kids.length < 2 || stranded.length)
            continue;
        used.delete(region.getAttribute("data-region"));
        region.removeAttribute("data-region");
        kids.forEach((kid) => {
            if (!kid.hasAttribute("data-region"))
                kid.setAttribute("data-region", regionName(kid, used));
        });
    }
}
...
export function repairImages(root, _doc) {
    root.querySelectorAll("img").forEach((img) => {
        const fix = () => {
            const seed = encodeURIComponent((img.alt || img.getAttribute("src") || "photo").split("/").pop()
        ...
        if (!/^https?:/i.test(src) || /\/seed\/SLUG\b/.test(src))
            fix();
        else
...
export function flash(el) {
    el.animate([{ opacity: 0.35, transform: "translateY(3px)" }, { opacity: 1, transform: "none" }], { duration: 220, easing: "ease-out" });
}
...
export function applyRegion(doc, id, html) {
    ...
    if (target.innerHTML === html)
        return; // already delivered in pieces
...
export function fieldKey(field) {
    ...
    if (field.name || field.id || said)
        return (field.name || field.id || said);
...
export function keySymbol(e, field) {
    ...
    if (e.key.length === 1)
        return field.type === "password" ? "•" : e.key;
    return KEY_SYMBOLS[e.key] ?? null;
}
export class KeyLog {
    stretches = []; // oldest first
    field = null; // the field the last stretch belongs to
    ...
    #stretch(field) {
        ...
        if (last && this.field === field && this.stretches.length > this.sent)
            return last;
        const region = field.closest("[data-region]")?.getAttribute("data-region");
    ...
    key(field, symbol) {
        ...
        stretch.keys += symbol;
        if (stretch.keys.length > MAX_KEYS)
            stretch.keys = "…" + stretch.keys.slice(-MAX_KEYS);
    }
    paste(field, text) {
...
export const REAL_CONTROLS = 'button, a[href], input[type="submit"], input[type="button"], input[type="reset"], [role="button"]';
...
export function isClickable(el, doc) {
    if (el.matches(REAL_CONTROLS) || el.matches(CLICK_INTENT))
        return true;
...
export function findControl(target, doc) {
    ...
    for (let depth = 0; el && el !== doc.body && depth < 4; el = el.parentElement, depth++) {
        if (/^(INPUT|TEXTAREA|SELECT|LABEL|OPTION)$/.test(el.tagName))
            return null;
        if (el.matches(CONTROL_SELECTOR) || isClickable(el, doc))
            return el;
    }
    return null;
}
