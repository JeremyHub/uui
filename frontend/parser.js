// Incremental parser for the #plan / #region / #screen / #end reply format.
//
...
export class PatchParser {
  constructor() {
    this.buf = "";
    this.kind = null;        // "region" | "screen" | null
    this.regionId = null;
    this.body = [];
    this.unflushed = "";     // region text not yet complete enough to send
  }
  ...
  *finish() {
  ...
  #flush() {
    ...
    this.unflushed = "";
    let html = cleanOutput(this.body.join(""));
    let event = null;
    if (html) {
      if (this.kind === "screen") {
  ...
  *#line(line) {
    ...
    if (stripped.startsWith("#plan")) {
      ...
      if (event) yield event;
      this.kind = "region";
      this.regionId = stripped.slice(7).trim().replace(/^["']|["']$/g, "") || "main";
      this.body = [];
      this.unflushed = "";
      yield { type: "region_open", id: this.regionId };
