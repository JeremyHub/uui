...
// about the model.
//
// The elements here belong to the iframe's window, not this one, so `instanceof
// HTMLInputElement` is false for every one of them. They are told apart by tag name,
// and cast to what the tag says they are.

/** One of FIELDS, typed as an input. The tag name says which properties are real. */
export type Field = HTMLInputElement;

export type FormState = Record<string, unknown>;

/** A run of keys pressed in one field -- see KeyLog. */
...
/** What was clicked, submitted or changed, as the model is told about it. */
...
/** Something the user did that the model gets a turn to answer. */
...
export function errorRegion(message: unknown): string {
...
export function runScripts(root: ParentNode, doc: Document): void {
...
export function regionHost(doc: Document): Element {
...
export function ensureRegions(doc: Document): void {
...
export function regionName(el: Element, used: Set<string>): string {
...
export function splitOversizedRegions(doc: Document): void {
...
export function repairImages(root: ParentNode, _doc?: Document): void {
...
export function regionTarget(doc: Document, id: string): Element {
...
export function openRegion(doc: Document, id: string): void {
  const target = regionTarget(doc, id);
  target.innerHTML = "";
  flash(target);
}

export function appendToRegion(doc: Document, id: string, html: string): void {
  const target = regionTarget(doc, id);
...
export function applyRegion(doc: Document, id: string, html: string): void {
  const target = regionTarget(doc, id);
...
export function applyScreen(doc: Document, html: string): void {
...
export const maskSecret = (value: string): string => (value ? `(${value.length} characters, hidden)` : "");

/** What the user has put in a field, read from the field rather than its markup. */
export function fieldValue(field: Field): string | boolean | string[] {
  ...
  if (field.tagName === "SELECT") {
    const select = field as Element as HTMLSelectElement;
    const chosen = [...select.selectedOptions].map((o) => o.value || (o.textContent ?? "").trim());
...
export function collectFormState(container: ParentNode): FormState {
  ...
  container.querySelectorAll<Field>(FIELDS).forEach((field) => {
  ...
  container.querySelectorAll('[aria-pressed], [aria-checked]:not(input), [aria-selected="true"]')
    .forEach((el) => {
      const key = (el as Field).name || el.id || el.getAttribute("aria-label") ||
...
export function describeElement(el: Element): ElementData {
  ...
  return {
    ...
    inRegion: region ? region.getAttribute("data-region") ?? undefined : undefined,
    ...
    ...(el.matches(FIELDS) ? { label: fieldKey(field), value: fieldValue(field) } : {}),
    ...(Object.keys(data).length ? { data } : {}),
  };
}

/** Whether Enter in this element means "go": a one-line field. */
export function submitsOnEnter(el: Element): boolean {
  const field = el as Field;
  return el.tagName === "INPUT" && !NOT_INPUT.test(field.type) && !/^(checkbox|radio|range|color)$/.test(field.type);
}
...
export function keySymbol(e: KeyboardEvent, field: Field): string | null {
...
export class KeyLog {
  stretches: Stretch[] = [];   // oldest first
  field: Field | null = null;  // the field the last stretch belongs to
  sent = 0;                    // how many stretches the turn in flight has taken

  #stretch(field: Field): Stretch {
    ...
    const region = field.closest("[data-region]")?.getAttribute("data-region");
    const stretch: Stretch = { in: fieldKey(field), ...(region ? { region } : {}), keys: "" };
 ...
 */
export function changeIsRequest(field: Field): boolean {
...
export function allControls(doc: Document): Element[] {
...
export function isClickable(el: Element, doc: Document): boolean {
...
export function findControl(target: Element, doc: Document): Element | null {
...
export function isLocal(el: Element): boolean {
