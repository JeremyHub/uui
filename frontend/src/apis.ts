...
export const KNOWN_APIS: [url: string, what: string][] = [
...
export function apiCatalogue(): string {
 ...
 * makes.
 */
export function compactData(value: unknown, depth = 0): unknown {
  if (Array.isArray(value)) {
    const head: unknown[] = value.slice(0, MAX_ARRAY_ITEMS).map((v) => compactData(v, depth + 1));
    if (value.length > MAX_ARRAY_ITEMS) head.push(`…${value.length - MAX_ARRAY_ITEMS} more`);
    return head;
  }
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([k, v]) => [k, compactData(v, depth + 1)]),
    );
  }
...
/** What /fetch answers with, and what a direct fetch is made to look like. */
...
export async function fetchForPrompt(url: string): Promise<string> {
  let result: FetchResult | null;
  try {
    const viaServer = await fetch(`fetch?url=${encodeURIComponent(url)}`);
    result = viaServer.ok ? await viaServer.json() as FetchResult : null;
  } catch {
    result = null;
  }
  ...
  if (!result) {
    ...
    try {
      ...
      result = { url, error: `Could not fetch it from the browser either (${(e as Error).message}).` };
    }
  }

  if (result.error) return `DATA FROM ${url}:\n(unavailable: ${result.error})`;
