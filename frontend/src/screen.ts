...
export const elision = (n: number, tag: string): string =>
...
const signature = (el: Element): string => `${el.tagName.toLowerCase()}.${el.getAttribute("class") || ""}`;

function truncate(text: string, limit: number): string {
  return text.length <= limit ? text : text.slice(0, limit) + "…";
}

const attr = (value: unknown): string => String(value).replace(/&/g, "&amp;").replace(/"/g, "&quot;");
...
function liveState(node: Element): Record<string, string | boolean> | null {
  ...
  if (tag === "INPUT") {
    ...
    if (input.type === "checkbox" || input.type === "radio") return { checked: input.checked };
    if (/^(submit|button|reset|image|file)$/.test(input.type)) return null;
    return { value: input.type === "password" ? maskSecret(input.value) : input.value };
  }
  if (tag === "OPTION") return { selected: (node as HTMLOptionElement).selected };
  return null;
}

function serialize(node: Node, out: string[], keep: number, root: Element): void {
  ...
  if (tag === "textarea") {
    out.push(truncate((el as HTMLTextAreaElement).value, MAX_TEXT), "</textarea>");
    return;
  }
...
function serializeChildren(nodes: NodeListOf<ChildNode>, out: string[], keep: number, root: Element): void {
  let runSig: string | null = null, runN = 0;
  ...
  for (const child of nodes) {
    if (child.nodeType === Node.ELEMENT_NODE) {
      const sig = signature(child as Element);
      if (sig === runSig) {
        runN += 1;
...
export function compact(el: Element, keep = DEFAULT_KEEP): string {
  const out: string[] = [];
...
export function regionsOf(doc: Document): Element[] {
 ...
 */
export function renderScreen(doc: Document, budget = 7000): string {
