...
//
// The protocol lives in turn.ts and the page handling in dom.ts; this is the part that
// knows there is a user watching. Nothing here is specific to where the model runs.

import {
  type Action, type Field, FIELDS, KeyLog, LOCAL_GRACE_MS, allControls, appendToRegion, applyRegion, applyScreen,
...
import { SHELL_MAX_TOKENS, type TurnEvent, nameAddress, runPatch, runShell } from "./turn.js";
import {
  type Engine, type GpuCapability, type Host, type ModelInfo,
  createEngine, pickModel, serverHost, tabHost, webGPUCapability,
} from "./engine.js";

// Every element looked up here is in index.html, so none of them can be missing.
const appEl = document.getElementById("app") as HTMLIFrameElement;
const statusEl = document.getElementById("status")!;
const transcriptEl = document.getElementById("transcript")!;
const speculateEl = document.getElementById("speculate") as HTMLInputElement;
const hostEl = document.getElementById("engine") as HTMLSelectElement;
...
// The iframe's document and window. Always there: the frame never leaves this origin.
const frameDoc = (): Document => appEl.contentDocument!;
const frameWin = (): Window => appEl.contentWindow!;

const delegated = new WeakSet<Document>();   // documents whose click/submit delegation is live
...
const frameReady = new Promise<void>((resolve) => {
  appEl.addEventListener("load", function loaded() {
    if (frameWin().location.protocol !== "blob:") return;
...
const log: Record<string, unknown>[] = [];   // debug transcript
let busy = false;
let keyLog = new KeyLog();         // keys pressed in the page since the model last saw them
// The update in flight, while there is one. Until the model starts writing, nothing on
// screen has changed, so a newer request can replace it.
let turn: { abort: AbortController; rendering: boolean; done: Promise<void> } | null = null;
// The first screen while it is being written. A new session stops it.
let shellRun: { abort: AbortController; done: Promise<void> } | null = null;
let actionSeq = 0;
...
const loading = {
  ...
  show(title: string, { detail = "", subtle = false, delay = 0 } = {}) {
  ...
  indeterminate() { loadingBar.classList.add("indeterminate"); loadingFill.style.width = ""; },
  progress(fraction: number) {
    loadingBar.classList.remove("indeterminate");
    loadingFill.style.width = `${Math.min(100, Math.max(0, fraction * 100))}%`;
  },
  hide() {
    clearTimeout(loadingTimer);
...
let timerId: ReturnType<typeof setInterval> | undefined, timerStart = 0, timerLabel = "";

function setStatus(text: string, extra = "") {
  statusEl.innerHTML = "";
...
function documentHead(title: string) {
...
function option(value: string, label: string, extra: Partial<HTMLOptionElement> = {}) {
...
/** A notice's text: plain strings, and codes to show as copyable code. */
type NoticePart = string | { code: string };
...
function gpuNotice(gpu: GpuCapability): Notice | null {
  ...
  const flag = (name: string) => ({ code: `${scheme}://flags/#${name}` });
...
function showNotice(notice: Notice | null) {
...
function populateModels(known: ModelInfo[] | null = null) {
  modelsReady = fillModels(known);
  return modelsReady;
}

async function fillModels(known: ModelInfo[] | null) {
...
async function ensureEngineReady(): Promise<Engine> {
...
async function setupEngineChoice() {
  ...
  hostEl.addEventListener("change", () => { dropEngine(); void populateModels().then(warmUp); });
...
const predictions = new Map<string, TurnEvent[]>();    // signature -> events, once a guess is complete
...
let specAbort: AbortController | null = null;
let specRun = 0;
let hoverTimer: ReturnType<typeof setTimeout> | undefined;

function signature(action: Action) {
...
function speculationTargets(doc: Document) {
...
async function generatePrediction(doc: Document, el: Element, myRun: number) {
  const action: Action = { event: "click", elementData: describeElement(el), formValues: collectFormState(doc) };
  ...
  try {
    ...
    if ((e as Error | null)?.name !== "AbortError") console.warn("speculation failed", e);
    return false;
  } finally {
    specAbort = null;
  }
...
async function speculate(doc: Document) {
  if (!engine) return;
  const myRun = ++specRun;
...
function predictOnHover(doc: Document, el: Element) {
  ...
  hoverTimer = setTimeout(() => {
    if (busy) return;
    cancelSpeculation();
    void generatePrediction(doc, el, specRun);
  }, HOVER_DELAY_MS);
}

// --- turns ------------------------------------------------------------------

function applyEvent(doc: Document, event: TurnEvent, touched?: string[]) {
...
function applyEvents(doc: Document, events: TurnEvent[]) {
  let plan = "";
...
async function sendAction(action: Action) {
  ...
  try {
    ...
    for await (const event of runPatch({
      engine: ready, doc, concept, action, memory: journal.toPrompt(), address, signal: current.abort.signal,
    })) {
      ...
      written += ("html" in event ? event.html : "").length;
    ...
    if ((e as Error | null)?.name !== "AbortError") applyRegion(doc, "uui-error", errorRegion((e as Error).message));
  } finally {
    loading.hide();
  }
...
async function startFromConcept(options: StartOptions = {}) {
  if (busy) return;
...
async function firstScreen({ prompt, address: typed }: StartOptions = {}, signal: AbortSignal) {
  concept = (prompt ?? conceptEl.value).trim() || "something interesting";
  enterSession();
  busy = true;
  ...
  try {
    ...
    await modelsReady;
    await frameReady;
    ready = await ensureEngineReady();
  } catch (e) {
    loading.hide();
    setStatus(`could not start: ${(e as Error).message}`);
    leaveSession();
    busy = false;
    return;
  }
  ...
  if (typed) {
    ...
    try {
      setAddress(await nameAddress({ engine: ready, concept, signal }));
    } catch (e) {
  ...
  frameWin().addEventListener("error", () => { scriptFailed = true; }, true);

  doc.write(documentHead(concept));
  let written = 0;
  try {
    for await (const event of runShell({ engine: ready, concept, address, signal })) {
    ...
    doc.write(errorRegion((e as Error).message));
  }
  ...
  renderTranscript();
  busy = false;
  if (!signal.aborted) void speculate(doc);
}

// --- input capture ----------------------------------------------------------

function attachDelegation(doc: Document) {
  if (delegated.has(doc)) return;
  delegated.add(doc);

  doc.addEventListener("click", (e) => {
    const el = findControl(e.target as Element, doc);
    if (!el) return;
    if (isLocal(el)) {
      ...
      if ((el as Field).type === "submit" && (el as Field).form) return;
      // Let the page's own handler run, but never let a real href navigate away.
      if (el.tagName === "A") e.preventDefault();
      const before = doc.body.innerHTML;
      setTimeout(() => {
        if (doc.body.innerHTML !== before) return;
        void sendAction({ event: "click", elementData: describeElement(el), formValues: collectFormState(doc) });
      }, LOCAL_GRACE_MS);
      return;
    }
    e.preventDefault();
  ...
  doc.addEventListener("keydown", (e) => {
    ...
    setTimeout(() => {
      if (doc.body.innerHTML !== before) return;
      void sendAction({ event: "submit", elementData: describeElement(field), formValues: collectFormState(doc) });
    }, LOCAL_GRACE_MS);
  }, true);
  ...
  doc.addEventListener("paste", (e) => {
    const field = (e.target as Element).closest?.<Field>(FIELDS);
  ...
  doc.addEventListener("change", (e) => {
    ...
    setTimeout(() => {
      if (doc.body.innerHTML !== before) return;
      void sendAction({ event: "change", elementData: describeElement(field), formValues: collectFormState(doc) });
    }, LOCAL_GRACE_MS);
  }, true);
...
function interceptNavigation(e: NavigateEvent) {
  if (e.destination.sameDocument || !e.cancelable) return;
  e.preventDefault();
  void sendAction({
    event: "navigate",
    elementData: { tag: "navigation", text: "", href: e.destination.url },
    formValues: collectFormState(frameDoc()),
  });
}

function guardNavigation(win: Window | null) {
...
document.getElementById("start-btn")!.addEventListener("click", () => void startFromConcept());
conceptEl.addEventListener("keydown", (e) => {
  if (e.key === "Enter") void startFromConcept();
});
...
function toAddress(text: string, base = address) {
  const raw = text.trim();
...
async function startNewSession(prompt: string) {
  ...
  void startFromConcept({ prompt, address: typed });
}
...
document.getElementById("home-btn")!.addEventListener("click", () => {
  if (document.body.classList.contains("at-home")) conceptEl.focus();
  else location.reload();
});

document.getElementById("examples")!.addEventListener("click", (e) => {
  const chip = (e.target as Element).closest<HTMLElement>("[data-concept]");
  if (!chip) return;
  conceptEl.value = chip.dataset.concept ?? "";
  void startFromConcept();
});
...
if (aboutEl.hidden) conceptEl.focus();
