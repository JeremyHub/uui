...
/** What the parser hands on, in the order the reply produced it. */
export type PatchEvent =
  | { type: "plan"; text: string }
  | { type: "url"; url: string }
  | { type: "region_open"; id: string }
  | { type: "region_chunk"; id: string; html: string }
  | { type: "region"; id: string; html: string; demoted?: boolean }
  | { type: "screen"; html: string }
  | { type: "none" };
 ...
 */
export function cleanOutput(html: string): string {
 ...
 */
export function splitCompleteElements(text: string): { finished: string[]; remainder: string } {
  const finished: string[] = [];
  let start = 0, depth = 0, match: RegExpExecArray | null;
  TAG_RE.lastIndex = 0;
...
export class PatchParser {
  buf = "";
  kind: "region" | "screen" | null = null;
  regionId: string | null = null;
  body: string[] = [];
  unflushed = "";     // region text not yet complete enough to send

  *feed(text: string): Generator<PatchEvent> {
    this.buf += text;
    let nl: number;
  ...
  *#line(line: string): Generator<PatchEvent> {
    const stripped = line.trim();
    if (FENCE_LINE_RE.test(line)) return;
  ...
  *#consumeBody(text: string): Generator<PatchEvent> {
    this.body.push(text);
    ...
    for (const piece of finished) {
      ...
      if (chunk) yield { type: "region_chunk", id: this.regionId!, html: chunk };
    }
  }
}
