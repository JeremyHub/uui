...
import type { Action } from "./dom.js";
import type { ChatModel } from "./engine.js";
import { ADDRESS_SYSTEM_PROMPT, PATCH_SYSTEM_PROMPT, SHELL_SYSTEM_PROMPT } from "./prompts.js";
import { PatchParser, type PatchEvent } from "./parser.js";
import { regionsOf, renderScreen } from "./screen.js";

/** Everything a turn can emit: the parser's events, and the turn's own. */
export type TurnEvent =
  | PatchEvent
  | { type: "phase"; name: string }
  | { type: "fetching"; url: string }
  | { type: "screen_delta"; text: string }
  | { type: "screen_end" }
  | { type: "error"; message: string };

/** Where a data request's URL is reported, since it has to be acted on before the reply. */
interface DataRequest {
  url?: string;
}

type GetData = (url: string) => Promise<string>;
...
async function* resume<T>(first: IteratorResult<T>, iterator: AsyncIterator<T>): AsyncGenerator<T> {
 ...
 */
export function bodyContentStart(text: string): number {
...
export async function* runShell({ engine, concept, address, signal, getData = fetchForPrompt }: {
  ...
  const ask = (u: string, req: DataRequest) => replyStream({
 ...
 */
export function guardScreen(event: PatchEvent, { doc, action }: { doc: Document; action?: Action }): PatchEvent {
...
export async function* runPatch({
  ...
  const run = async function* (message: string, request: DataRequest): AsyncGenerator<TurnEvent> {
