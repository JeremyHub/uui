...
import type * as WebLLM from "https://esm.run/@mlc-ai/web-llm@0.2.85";
import { WEBLLM_URL, keepGpuJobsShort } from "./gpu-jobs.js";

// --- the shapes every host shares -----------------------------------------------

/** A model a host can run. sizeMB is null where the host does not say. */
...
/** The OpenAI chat completions shape, which WebLLM's engine already speaks. */
export type Backend = Pick<WebLLM.MLCEngineInterface, "chat" | "interruptGenerate"> & {
  unload?: () => Promise<void>;
};
...
export interface Host {
  ...
  load(modelId: string, onProgress?: (progress: LoadProgress) => void): Promise<Backend>;
}
...
/** Anything that streams a reply -- an Engine, or a stand-in for one in a test. */
export interface ChatModel {
  chat(request: ChatRequest): AsyncIterable<string>;
}

export interface Engine extends ChatModel {
  host: string;
  model: string;
  label: string;
 ...
 */
export function pickModel(models: ModelInfo[], budgetMB: number): ModelInfo | null {
  const affordable = models.filter(
    (m): m is ModelInfo & { sizeMB: number } => m.sizeMB !== null && m.sizeMB <= budgetMB,
  );
...
export function createEngine(host: Host, modelId: string): Engine {
  let backend: Backend | null = null;
  let loading: Promise<void> | null = null;  // the load in flight, shared by everyone who asks for it
  let reportProgress: ((progress: LoadProgress) => void) | null = null; // whoever asked most recently is the one watching
  let disposed = false;
  ...
  return {
    ...
    // Loading can start before anyone is watching (see warmUp in main.ts), so a second
    ...
    async *chat({ system, user, maxTokens, temperature, signal }) {
      ...
      let stream: AsyncIterator<WebLLM.ChatCompletionChunk, undefined> | null = null;
      let finished = false;
      ...
      try {
        ...
        if (/device.*lost|disposed/i.test((e as Error | null)?.message ?? "")) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-base-to-string -- whatever was thrown, as text
 ...
 */
function serverBackend(endpoint: string, model: string): Backend {
  const inFlight = new Set<AbortController>();

  async function* completions(
    request: WebLLM.ChatCompletionRequest, controller: AbortController,
  ): AsyncGenerator<WebLLM.ChatCompletionChunk> {
    try {
      ...
      if (!response.body) throw new Error("Ollama sent no reply");
      ...
      while (true) {
        ...
        while ((nl = buf.indexOf("\n")) !== -1) {
          ...
          buf = buf.slice(nl + 1);
          if (!line) continue;
          const chunk = JSON.parse(line) as { error?: string; message?: { content?: string } };
...
export function serverHost(
  { chatEndpoint = "chat", modelsEndpoint = "models", budgetMB = Infinity } = {},
): Host {
  return {
    id: "server",
    label: "This machine",
    ...
    async listModels() {
      ...
      const data = await response.json() as {
        models?: { id: string; sizeMB?: number | null; metered?: boolean }[];
      };
...
let webllmModule: typeof WebLLM | null = null;

async function loadWebLLM(): Promise<typeof WebLLM> {
  if (!webllmModule) webllmModule = await import(/* @vite-ignore */ WEBLLM_URL) as typeof WebLLM;
  return webllmModule;
}
...
/** What the tab's graphics card can do. `why` says what is missing when it is not ok. */
...
export async function webGPUCapability(): Promise<GpuCapability> {
  ...
  try {
    ...
    return { ok: false, budgetMB: 0, why: `WebGPU could not start (${(e as Error).message}).` };
  }
}

/** The prebuilt entries this device could actually run, smallest first. */
export function usableModels(entries: WebLLM.ModelRecord[], { f16 = true } = {}): ModelInfo[] {
  return entries
    .map((m) => ({
      id: m.model_id,
...
// mid-range card, past the point where the driver resets the GPU. See gpu-jobs.ts.
const PREFILL_CHUNK_TOKENS = 128;

export function tabHost(
  gpu: GpuCapability, { prefillChunkTokens = PREFILL_CHUNK_TOKENS, useWorker = true } = {},
): Host {
  return {
    id: "tab",
    ...
    async load(modelId, onProgress) {
      const webllm = await loadWebLLM();
      const config: WebLLM.MLCEngineConfig = {
      ...
      // rendering what it just decoded -- see llm-worker.ts. A file:// page cannot start
