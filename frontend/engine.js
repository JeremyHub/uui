...
export function pickModel(models, budgetMB) {
    ...
    if (!affordable.length)
        return models.find((m) => m.lowResource) ?? models[0] ?? null;
    const rank = (m) => {
        if (/qwen.*coder/i.test(m.id))
            return 3; // best at structured markup for its size
        if (/qwen/i.test(m.id))
            return 2;
        if (/llama-?3|phi-?3|gemma-?2/i.test(m.id))
            return 1;
        return 0;
    };
...
export function createEngine(host, modelId) {
    ...
    async function load() {
        ...
        if (disposed)
            Promise.resolve(loaded.unload?.()).catch(() => { });
        else
            backend = loaded;
    }
    return {
...
function serverBackend(endpoint, model) {
    ...
    async function* completions(request, controller) {
        try {
            ...
            if (!response.body)
                throw new Error("Ollama sent no reply");
...
async function loadWebLLM() {
    if (!webllmModule)
        webllmModule = await import(/* @vite-ignore */ WEBLLM_URL);
    return webllmModule;
}
/**
...
export function budgetFromLimits(limits = {}) {
    const perBufferMB = Math.max((limits.maxBufferSize ?? 0) / (1024 * 1024), (limits.maxStorageBufferBindingSize ?? 0) / (1024 * 1024));
    if (perBufferMB <= 0)
        return 2000;
...
export async function webGPUCapability() {
    ...
    try {
        ...
        const software = /swiftshader|lavapipe|llvmpipe|software/i.test(`${info.architecture ?? ""} ${info.description ?? ""} ${info.vendor ?? ""}`);
        return {
            ok: true,
            software,
