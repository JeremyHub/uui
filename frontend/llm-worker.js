...
// it arrive before onmessage exists. Must match WEBLLM_URL in gpu-jobs.js.
...
handler.engine.reload = async (modelIds, chatOptions) => {
  await reload(modelIds, chatOptions);
  const options = [chatOptions ?? []].flat()[0];
  keepGpuJobsShort(handler.engine, options?.prefill_chunk_size);
};
