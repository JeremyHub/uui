...
import { FIELDS, KeyLog, LOCAL_GRACE_MS, allControls, appendToRegion, applyRegion, applyScreen, changeIsRequest, collectFormState, describeElement, ensureRegions, errorRegion, findControl, isClickable, isLocal, keySymbol, openRegion, repairImages, runScripts, splitOversizedRegions, submitsOnEnter, } from "./dom.js";
...
const frameDoc = () => appEl.contentDocument;
const frameWin = () => appEl.contentWindow;
...
const frameReady = new Promise((resolve) => {
    appEl.addEventListener("load", function loaded() {
        if (frameWin().location.protocol !== "blob:")
            return;
...
const loadingFill = loadingBar.firstElementChild;
let loadingTimer;
const loading = {
    ...
    show(title, { detail = "", subtle = false, delay = 0 } = {}) {
        ...
        if (delay)
            loadingTimer = setTimeout(reveal, delay);
        else
            reveal();
        this.indeterminate();
    },
...
// --- status -----------------------------------------------------------------
let timerId, timerStart = 0, timerLabel = "";
...
async function fillModels(known) {
    ...
    modelEl.replaceChildren(option("auto", auto ? `Auto · ${auto.id}` : "Auto"), ...models.map((m) => {
        const el = option(m.id, m.sizeMB ? `${m.id} (${(m.sizeMB / 1024).toFixed(1)} GB)` : m.id, { disabled: host.strictBudget && m.sizeMB !== null && m.sizeMB > host.budgetMB * 1.3 });
        if (m.metered)
            el.dataset.metered = "";
        return el;
    }));
...
function buildEngine() {
    ...
    if (!modelId)
        throw new Error(`no model is available ${currentHost().label.toLowerCase()}`);
...
async function setupEngineChoice() {
    ...
    modelEl.addEventListener("change", () => {
        ...
        if (modelEl.selectedOptions[0]?.dataset.metered !== undefined)
            speculateEl.checked = false;
        dropEngine();
        void warmUp();
    });
...
async function warmUp() {
    if (engine || busy || !currentHost()?.warmUp)
        return;
    try {
        const candidate = buildEngine();
        if (engine || !(await candidate.isCached()))
            return;
        engine = candidate;
...
async function generatePrediction(doc, el, myRun) {
    ...
    try {
        ...
        if (e?.name !== "AbortError")
            console.warn("speculation failed", e);
        return false;
    }
    finally {
        specAbort = null;
...
function applyEvent(doc, event, touched) {
    ...
    else if (event.type === "region_open")
        openRegion(doc, event.id);
    else if (event.type === "region_chunk")
        appendToRegion(doc, event.id, event.html);
    else if (event.type === "region") {
        applyRegion(doc, event.id, event.html);
        touched?.push(event.id);
    }
    else if (event.type === "screen") {
        applyScreen(doc, event.html);
        touched?.push("(whole screen)");
    }
    else if (event.type === "error")
        applyRegion(doc, "uui-error", errorRegion(event.message));
...
async function sendAction(action) {
    ...
    try {
        ...
        for await (const event of runPatch({
            ...
            if (event.type === "plan") {
                plan = event.text;
                timerLabel = plan.slice(0, 80);
                loading.detail(plan);
            }
            else if (event.type === "phase" || event.type === "none")
                continue;
            ...
            if (written)
                loading.progress(Math.min(0.95, written / 1400));
        }
    }
    catch (e) {
        if (e?.name !== "AbortError")
...
async function firstScreen({ prompt, address: typed } = {}, signal) {
    ...
    if (!address)
        setAddress(`https://${slugOf(concept)}.app/`);
    const doc = frameDoc();
    ...
    if (!written && !signal.aborted)
        doc.write(errorRegion("Nothing came back. Try asking another way."));
    doc.write("</body></html>");
    doc.close();
    try {
        if (scriptFailed)
...
function attachDelegation(doc) {
    ...
    doc.addEventListener("submit", (e) => {
        ...
        if (isLocal(form) || isLocal(e.submitter || form)) {
            const before = doc.body.innerHTML;
            setTimeout(() => { if (doc.body.innerHTML === before)
                send(); }, LOCAL_GRACE_MS);
            return;
        }
        e.stopImmediatePropagation();
    ...
    doc.addEventListener("keydown", (e) => {
        ...
        if (!submitsOnEnter(field) || field.form)
            return;
        const before = doc.body.innerHTML;
        setTimeout(() => {
            if (doc.body.innerHTML !== before)
    ...
    doc.addEventListener("keydown", (e) => {
        const field = e.target;
        if (e.isComposing || !field.matches?.(FIELDS))
            return;
        const symbol = keySymbol(e, field);
        if (symbol)
            keyLog.key(field, symbol);
    }, true);
    doc.addEventListener("paste", (e) => {
        const field = e.target.closest?.(FIELDS);
        if (field)
            keyLog.paste(field, e.clipboardData?.getData("text") ?? "");
    }, true);
    ...
    doc.addEventListener("change", (e) => {
        const field = e.target;
        if (!field.matches?.(FIELDS) || !changeIsRequest(field))
            return;
        const before = doc.body.innerHTML;
        setTimeout(() => {
            if (doc.body.innerHTML !== before)
...
function interceptNavigation(e) {
    if (e.destination.sameDocument || !e.cancelable)
        return;
    e.preventDefault();
    void sendAction({
        event: "navigate",
...
function toAddress(text, base = address) {
    ...
    try {
        if (/^[/?#]/.test(raw))
            return base ? new URL(raw, base).href : null;
...
async function startNewSession(prompt) {
    ...
    if (shellRun) {
        shellRun.abort.abort();
        await shellRun.done.catch(() => { });
    }
    journal = new Journal();
    keyLog = new KeyLog();
    log.length = 0;
...
aboutEl.addEventListener("click", (e) => { if (e.target === aboutEl)
    showAbout(false); });
document.addEventListener("keydown", (e) => { if (e.key === "Escape" && !aboutEl.hidden)
    showAbout(false); });
